...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/island-guide/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'dist/island-guide/browser'), 'index.html')
    )
    .toString();
  ...
  server.get('*', (req, res) => {
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          params.indexOf('guide/') > -1 ||
          params.indexOf('destination/') > -1
        ) {
          let id = params.split('/');
          id = id[id.length - 1];
          ...
          const talent = {
            name: `IslandGuide.Tours Research ${group?.PageTitle}`,
